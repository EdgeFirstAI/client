...
- Rust 2024 edition — use modern syntax and import conventions
...
- [ ] Code follows project style guidelines (check `rustfmt.toml`)
...
python3 -m slipcover --xml --out coverage.xml -m xmlrunner discover -s test -p "test*.py" -o target/python
...
- `.github/copilot-instructions.md` - AI assistant guidelines (this file)
...
- Uses `rustls` backend for TLS (via reqwest)
...
```bash
make pre-commit    # format + lint + build in one command
```
...
3. **Run code quality checks** (`make format && make lint`) or manually:
...
6. **Stable Rust**: Use `cargo fmt` for formatting (no nightly required)
7. **Pylance type narrowing**: After `self.assertIsNotNone(x)`, add `assert x is not None`
8. **Python test directory**: Tests are in `test/`, not the repository root
9. **Rust edition**: Project uses edition 2024 — use modern syntax accordingly
...
**This document version**: 2.1 (March 2026)
