...
class TestTasks(unittest.TestCase):
    ...
    def test_tasks(self):
        ...
        # Update stage
        client.update_stage(task.id, "download", "running", "Downloading dataset", 50)
    ...
    def test_sequence_upload_and_verification(self):
        ...
        try:
            ...
            annset_id = client.create_annotation_set(
                dataset_id, "Test Annotations", None
            )
            ...
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                for i in range(1, 11):
                    ...
                    sample.add_file(
                        SampleFile(file_type="image", filename=str(img_path))
                    )

                    seq1_samples.append(sample)

                results1 = client.populate_samples(
                    dataset_id, annset_id, seq1_samples, None
                )
                self.assertEqual(
                    len(results1), 10, "Should upload 10 samples for sequence 1"
                )
                ...
                results2 = client.populate_samples(
                    dataset_id, annset_id, seq2_samples, None
                )
                self.assertEqual(
                    len(results2), 10, "Should upload 10 samples for sequence 2"
                )
                ...
                exclamation_sample.add_file(
                    SampleFile(file_type="image", filename=str(exclamation_path))
                )

                client.populate_samples(
                    dataset_id, annset_id, [exclamation_sample], None
                )
                ...
                question_sample.add_file(
                    SampleFile(file_type="image", filename=str(question_path))
                )
                ...
                self.assertEqual(
                    len(seq1_samples_result), 10, "Should have 10 samples in sequence 1"
                )
                self.assertTrue(
                    all(s.sequence_uuid == seq1_uuid for s in seq1_samples_result),
                    "All sequence 1 samples should have same UUID",
                )
                seq1_frames = sorted(
                    [s.frame_number for s in seq1_samples_result if s.frame_number]
                )
                self.assertEqual(
                    seq1_frames,
                    list(range(1, 11)),
                ...
                self.assertEqual(
                    len(seq2_samples_result), 10, "Should have 10 samples in sequence 2"
                )
                self.assertTrue(
                    all(s.sequence_uuid == seq2_uuid for s in seq2_samples_result),
                    "All sequence 2 samples should have same UUID",
                )
                seq2_frames = sorted(
                    [s.frame_number for s in seq2_samples_result if s.frame_number]
                )
                self.assertEqual(
                    seq2_frames,
                    list(range(1, 11)),
                ...
                non_seq_samples = [s for s in all_samples if s.sequence_name is None]
                self.assertEqual(
                    len(non_seq_samples), 2, "Should have 2 non-sequence samples"
                )
