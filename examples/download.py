...
if __name__ == "__main__":
    ...
    args.add_argument("--output", type=str, default="dataset",
                      help="Output directory")
    args.add_argument("--groups", type=str, default="val",
    ...
    for group in args.groups.split(","):
        ...
        with tqdm(total=0, desc=f'Fetching {group} Samples') as bar:
            ...
            samples = client.samples(dataset_id=dataset.id,
        ...
        with tqdm(total=0, desc=f"Downloading {group} Images") as bar:
            def progress(current, total):
                if total != bar.total:
                    bar.reset(total)
                bar.update(current - bar.n)
            client.download_dataset(dataset_id=dataset.id,
                                    groups=[group],
                                    types=[FileType.Image],
